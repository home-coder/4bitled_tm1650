...
struct timer_list new_alarm_timer[TIMER_LIST];
...
{
	struct timer_list *timer_node = (struct timer_list *)data;
	
	printk("---time is now!\n");
	/* TODO spangled and keep 10 seconds ! */
	
	/*hakuna uevent support */
#if TM1650_UEVENT
	
#endif
	
	mod_timer(timer_node, jiffies + 24 * 60 * 60 * HZ);
}
...
#define ASSERT_ARGS(a)
	...
	getnstimeofday(&ts);
	rtc_time_to_tm(ts.tv_sec, &tm);
	printk("---%lu : %lu\n", (unsigned long)tm.tm_min, (unsigned long)tm.tm_sec);

	now_time = tm.tm_min * 60 + tm.tm_sec;
	alarm_time = set_time - now_time;
	if (alarm_time < 0) {
		*pend = (unsigned long)-1 * HZ; //FIXME should set it 24 - xxx .
		return ;
	}
...
/*
 *Here : hakuna make  timers nodes, but max is 32, and setup .
 */
static void tm1650_time_alarm_work(struct work_struct *ws)
{
	...
	struct proc_time_list *tmpos;

	ptr = time_list_head.prev;
	tmpos = list_entry(ptr, struct proc_time_list, list);
	...
	data_to_jiffies(tmpos->data, ":", &pend);
	if (pend > 0) {
		/* set timer */
		struct timer_list *data = &new_alarm_timer[timer_nr];
		setup_timer(&new_alarm_timer[timer_nr], alarm_work_func, \
					(unsigned long)data);
		printk("pend = %lu\n", pend);
		new_alarm_timer[timer_nr].expires = jiffies + pend;
		add_timer(&new_alarm_timer[timer_nr]);
		timer_nr++;
		timer_nr %= TIMER_LIST;
	} else {
		/* del from list */
        ...
        const struct i2c_device_id *id)
{
	...
	*/	
	...
	/*11. uevent, use struct list to do it, or use the kernel method like \
		chang rui shi dai*/
#if TM1650_UEVENT
	// search kobject_uevent !
#endif

	/* 12.take a long time to do the poll relations, IIInclude userspace and \
		the kernel space*/
#if TM1650_POLL
	//TODO
#endif

	/* 13.do something, must use the sysfs*/
#if TM1650_SYSFS
	//TODO
#endif

	/* 14. just test my coding skills, use listseq_file interface, is a\
		abstract obj */
#if TM1650_LIST
	//TODO
#endif
